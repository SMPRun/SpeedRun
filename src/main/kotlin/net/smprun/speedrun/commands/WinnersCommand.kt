        ...
        plugin.foliaLib.scheduler.runAsync { _ ->
            try {
                ...
                list.winners.forEachIndexed { index, w ->
                    sender.sendMessage(Component.text("${index + 1}. ${w.username} (${w.uuid}) - time=${w.winTime}", NamedTextColor.WHITE))
                }
            } catch (e: Exception) {
        ...
        plugin.foliaLib.scheduler.runAsync { _ ->
            try {
                ...
                if (best == null) {
                    ...
                    sender.sendMessage(Component.text("Best time: ${best.username} - ${best.winTime}", NamedTextColor.GOLD))
                }
            } catch (e: Exception) {
